...
Approach number 2 is much more difficult, so for now we will go with number 1. The difference between the two approaches is only that second is more comfortable for end user, while first is easier for compiler developer. There is no difference at all, when it comes to expressive power. Also, the first approach will result in faster compilation times, but it should be negligible.
